...
function StokUpdate({ menuData, onMenuChange }) {
  ...
  useEffect(() => {
    if (menuData && onMenuChange) {
      // Menüden gelen değişiklikleri stok listesine yansıt
      const yeniStok = { ...stok };

      // Menüdeki tüm kategorileri kontrol et
      Object.keys(menuData).forEach(kategori => {
        if (menuData[kategori]) {
          // Menüdeki ürünleri stok listesine ekle
          menuData[kategori].forEach(menuUrunu => {
            const stokUrunu = stok[kategori]?.find(s => s.ad === menuUrunu.ad);
            if (!stokUrunu) {
              // Yeni ürün ekle
              const birim = (kategori === "İçecekler" || kategori === "Tatlılar") ? "adet" : "porsiyon";
              if (!yeniStok[kategori]) yeniStok[kategori] = [];
              yeniStok[kategori].push({
                ad: menuUrunu.ad,
                miktar: 50,
                minStok: 10,
                birim: birim
          ...
          // Stok listesinde olup menüde olmayan ürünleri çıkar
          if (yeniStok[kategori]) {
            yeniStok[kategori] = yeniStok[kategori].filter(stokUrunu =>
              menuData[kategori].some(menuUrunu => menuUrunu.ad === stokUrunu.ad)
            );
          }
        }
      });
  ...
  const miktarKaydet = () => {
    if (!duzenleMiktar || !duzenleMinStok || duzenleModal.kategori === "Tümü") return;
    const yeniStok = { ...stok };
    ...
    <div style={{ maxWidth: 1000, margin: "32px auto", background: "#f8f9fa", borderRadius: 16, boxShadow: "0 2px 12px #0001", padding: 32 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 8 }}>
        {gosterilecekUrunler.map((urun, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", background: "#fff", borderRadius: 10, boxShadow: "0 1px 6px #0001", padding: "12px 20px", gap: 20 }}>
