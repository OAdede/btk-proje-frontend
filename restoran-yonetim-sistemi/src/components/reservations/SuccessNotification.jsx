...
export default function SuccessNotification({ visible, onClose, reservationData }) {
  ...
  // Mesaj türüne göre başlık ve içerik
      ...
      return {
        ...
        content: (
          <div style={{ color: colors.textColor, fontSize: "1rem", lineHeight: "1.6" }}>
            <div>
              <strong>{reservationData.message}</strong>
            </div>
          </div>
        ),
      };
      ...
      return {
        ...
        content: (
          <div style={{ color: colors.textColor, fontSize: "0.9rem", lineHeight: "1.6" }}>
            {reservationData?.masaNo && (
              <div>
                <strong>Masa:</strong> {reservationData.masaNo}
              </div>
            )}
            <div>
              <strong>Müşteri:</strong> {reservationData?.ad} {reservationData?.soyad}
            </div>
            <div>
              <strong>Tarih:</strong> {reservationData?.tarih}
            </div>
            <div>
              <strong>Saat:</strong> {reservationData?.saat}
            </div>
            <div>
              <strong>Kişi Sayısı:</strong> {reservationData?.kisiSayisi}
            </div>
            {reservationData?.telefon && (
              <div>
                <strong>Telefon:</strong> {reservationData.telefon}
              </div>
            )}
            {reservationData?.email && (
              <div>
                <strong>E-mail:</strong> {reservationData.email}
              </div>
            )}
          </div>
        ),
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Başarı Mesajı veya Rezervasyon Detayları */}
        ...
        {/* Not: Vite projesinde styled-jsx yoksa `jsx` özniteliğini kullanmayın */}
        <style>{`
          @keyframes slideIn {
